...
<html>
   <head>
      <meta charset = "utf-8">
      <title>Review Minor Applications</title>
   ...
   <body>
      ...
      <h1><strong>Review Minor Applications</strong></h1>

      <form method = "post" action = "reviewMinorApplication.php">
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              for (let i=0; i < data.length; i++) {
                $('#studentNames').append("<option value = "+ data[i].studentID + "," + data[i].minorID + ">"
                        + data[i].firstName + " " + data[i].lastName + " - " + data[i].minorName + "</option>");
              }
            }
          });
        });
      ...
      <p>
        <input id "submitButton" class = "myButton" name = "submit" type = "submit" value = "Approve Minor" />
...
<?php
        ...
        if(isset($_POST['submit']))
        {
          ...
          $removeApp = "DELETE FROM `minorApplication` WHERE `minorApplication`.`studentID` = '$student'
          AND `minorApplication`.`minorID` = '$minor'";
          ...
          if(!$removeCheck = mysqliQuery($removeApp, MYSQLI_USE_RESULT))
          {
            ...
            echo "<p>Application removed from minorApplication</p>";
          }

          //query to insert the student and major into studentsMajor
          $insertMinor = "INSERT INTO `studentsMinor` (studentID, minorID) VALUES ('$student', '$minor')";

          if(!$insertCheck = mysqliQuery($insertMinor, MYSQLI_USE_RESULT))
          {
            print( "<p>Could not execute query!</p>" );
            die( mysql_error() . "</body></html>" );
          }
          else{
            echo "<p>Application added to studentsMinors</p>";
          }
